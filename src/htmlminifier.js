...
function cleanConditionalComment(comment, options) {
  return options.processConditionalComments ? comment.replace(/^(\[if\s([^\]]+)\]>)([\s\S]*?)(<!\[endif])$/, function(match, prefix, cond, text, suffix) {
    //console.error('ConditionalComment: ' + cond)
    if (options.processConditionalCssAuto) {
      cond = cond.replace(/\s+/g,'').toLowerCase();
      cond = /^ie([789]|1[01])$/.test(cond) ? cond : true;
    } else {
      cond = true;
    }
    return prefix + minify(text, options, cond) + suffix;
  }) : comment;
}
  ...
  Object.keys(values).forEach(function(key) {
    ...
    if (key === 'caseSensitive') {
      ...
      options.minifyCSS = function(text, type) {
        ...
        var saved_opt = {};
        if (type && (type == 'inline' || type == 'media' || /^ie([789]|1[01])$/.test(type))) {
          if (value.format && value.format.semicolonAfterLastProperty) {
            saved_opt.semicolonAfterLastProperty = value.format.semicolonAfterLastProperty;
            value.format.semicolonAfterLastProperty = false;
          }
          if (value.level && value.level['1']) {
            if (value.level['1'].regularComments) {
              saved_opt.regularComments = value.level['1'].regularComments;
              value.level['1'].regularComments = false;
            }
            if (value.level['1'].specialComments) {
              saved_opt.specialComments = value.level['1'].specialComments;
              value.level['1'].specialComments = 0;
            }
          }
          if (/^ie([789]|1[01])$/.test(type)) {
            saved_opt.compatibility = value.compatibility || '*';
            value.compatibility = type;
          }
        }
        var cleanCssOutput = new CleanCSS(value).minify(wrapCSS(text, type));
        if (saved_opt.semicolonAfterLastProperty) {
          value.format.semicolonAfterLastProperty = saved_opt.semicolonAfterLastProperty;
        }
        if (saved_opt.regularComments) {
          value.level['1'].regularComments = saved_opt.regularComments;
        }
        if (saved_opt.specialComments) {
          value.level['1'].specialComments = saved_opt.specialComments;
        }
        if (saved_opt.compatibility) {
          value.compatibility  = saved_opt.compatibility;
        }
      ...
      options.minifyJS = function(text, inline) {
        ...
        //if (!value.parse.toplevel) console.log(value);
    ...
    else if (key === 'minifySVG' && typeof value !== 'function') {
      if (!value) {
        return;
      }
      if (typeof value !== 'object') {
        ...
        for (let i = value.plugins.length - 1; i >= 0; i--) {
          if (typeof value.plugins[i] === 'object' && !value.plugins[i].name) {
            for (const [key, val] of Object.entries(value.plugins[i])) {
              if (typeof val !== 'object') {
                value.plugins.push({name:key, active:!!val});
              }else {
                value.plugins.push({name:key, active:true, params:val});
              }
            }
            value.plugins.splice(i, 1);
          }
        }
      }
      // Установка общих параметров
      // value = {multipass:true, floatPrecision:3, ...value};
      if (value.multipass === undefined) {
        value.multipass = true;
      }
      // Активация(настройка) плагинов
      value.plugins.unshift(
        'removeXMLNS',
        'convertStyleToAttrs',
        'removeHiddenElems',
        'sortAttrs',
      ...
      // Параметры для отдельных плагинов
      value.plugins.find((plugin, i) => {
        if (typeof plugin === 'string') {
          plugin = value.plugins[i] = {name:plugin, active:true};
        }
        switch (plugin.name) {

          case 'removeHiddenElems': { // НЕ удалять:  visibility:hidden; display:none; opacity:0;
            plugin.params = plugin.params || {};
            plugin.params = {isHidden:false,displayNone:false,opacity0:false, ...plugin.params};
            value.plugins[i] = plugin;
            break;
          }

          case 'removeAttrs': {       // xml:space="preserve"
            let S = ':';
            plugin.params = plugin.params || {};
            plugin.params.attrs = plugin.params.attrs || [];
            if (!Array.isArray(plugin.params.attrs)) {
               plugin.params.attrs = [plugin.params.attrs];
            }
            if (typeof plugin.params.elemSeparator === 'string') {
              S = plugin.params.elemSeparator;
            }
            plugin.params.attrs = ['svg'+S+'xml\\x3Aspace'+S+'preserve', ...plugin.params.attrs];
            value.plugins[i] = plugin;
            break;
          }
        }
      ...
      options.minifySVG = function(text, svgIDs) {
        if (!OptimSVG) {
          OptimSVG = require('svgo');
          value.plugins = OptimSVG.extendDefaultPlugins(value.plugins);
          if (svgIDs && svgIDs.length > 0) {
            //console.error(svgIDs);
            value.plugins.find((plugin, i) => {
              if (typeof plugin === 'string') {
                if (plugin == 'cleanupIDs') {
                  value.plugins[i] = {name:'cleanupIDs', active:true, params:{preserve:svgIDs}}
                  return true; // stop searching
                }
              } else if (plugin.name == 'cleanupIDs') {
                if (plugin.active !== false) {
                  plugin.params = plugin.params || {};
                  plugin.params.preserve = plugin.params.preserve || [];
                  plugin.params.preserve = [...plugin.params.preserve, ...svgIDs];
                  value.plugins[i] = plugin;
                }
                return true;   // stop searching
              }
        ...
        var result = OptimSVG.optimize(text, value);
        if (result.error) {
          options.log(result.error);
          return text;
        }
...
function minify(value, options, partialMarkup) {
  ...
  if (!partialMarkup && typeof options.minifySVG === 'function') {
    // Поиск href="#id" во всех <svg>, для защиты этих id от удаления плагином 'cleanupIDs'
    value.replace(/<svg\b[\s\S]+?<\/svg>/ig, function(svg) {
      svg.replace(/\bhref\s*=\s*["']?#([^\s'"\/>]+)/ig, function(m, id) {
        if (svgIDs.indexOf(id) == -1) {
          svgIDs.push(id);
        }
      });
    });
  ...
  new HTMLParser(value, {
    ...
    chars: function(text, prevTag, nextTag) {
      ...
      if (isStyleSheet(currentTag, currentAttrs)) {
        text = options.minifyCSS(text, partialMarkup);
      }
    ...
    svg: (typeof options.minifySVG !== 'function') ? false : function(html) {
      return html.replace(/^<svg\b[\s\S]+?<\/svg>/i, function(svg) {
        return options.minifySVG(svg, svgIDs);
      });
    }
