...
function optimizeComment(token, options) {
  if (options.level[OptimizationLevel.One].regularComments) {
    if ((options.level[OptimizationLevel.One].regularComments == 'all' || options.commentsKept < options.level[OptimizationLevel.One].regularComments)) {
      options.commentsKept++;
      return;
    }
  }else if (token[1][2] == Marker.EXCLAMATION && (options.level[OptimizationLevel.One].specialComments == 'all' || options.commentsKept < options.level[OptimizationLevel.One].specialComments)) {
    options.commentsKept++;
    return;
  }
